...
description: |
  ...
  Requires: curl, jq.
  Credentials: PROXMOX_HOST, PROXMOX_TOKEN_ID, PROXMOX_TOKEN_SECRET — set as env vars or stored in a cred file (sourced at runtime, user-created, mode 600).
  Cred file path: $PROXMOX_CRED_FILE (default ~/.proxmox-credentials). Fleet convention: ~/.openclaw/secrets/proxmox-<agent>.env.
  Writes: cred file is user-created, not generated by this skill.
  ...
  Configuration: $PROXMOX_CRED_FILE (default ~/.proxmox-credentials)
metadata: { "openclaw": { "emoji": "🖥️", "homepage": "https://github.com/captain-marlow/proxmox-skill", "requires": { "bins": ["curl", "jq"] }, "os": ["darwin", "linux"] } }
---
...
Create a credential file (default path `~/.proxmox-credentials`):
...
**Custom credential path** — set `PROXMOX_CRED_FILE` to override the default location. Useful for fleet/agent setups where each agent has its own named credential file:

```bash
# e.g. OpenClaw fleet convention
...
```bash
source "${PROXMOX_CRED_FILE:-$HOME/.proxmox-credentials}"
...
- **Credential file** (`$PROXMOX_CRED_FILE`, default `~/.proxmox-credentials`) is user-created, not auto-generated by this skill. Must be mode 600. Rotate tokens immediately if exposed
